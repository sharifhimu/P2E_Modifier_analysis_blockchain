...
module.exports = async function(callback) {
    try {
        console.log("🚀 Starting token minting process with RANDOM amounts...\n");

        // Get deployed contracts
        const myToken = await MyToken.deployed();
        const otherToken = await OtherToken.deployed();
        
        // Get owner account (first account)
        const accounts = await web3.eth.getAccounts();
        const owner = accounts[0];
        
        console.log(`📋 Owner account: ${owner}`);
        console.log(`📄 MyToken address: ${myToken.address}`);
        console.log(`📄 OtherToken address: ${otherToken.address}\n`);
        ...
        console.log(`📊 Skipping first ${startIndex} addresses (already have token0)`);
        console.log(`📊 Starting from address ${startIndex + 1}/${addresses.length}`);
        console.log(`📊 ${remainingAddresses.length} addresses remaining to mint\n`);

        // 🎲 Random amount configuration
        const minTokens = 100;    // Minimum tokens
        const maxTokens = 10000;  // Maximum tokens

        console.log(`🎲 Random range: ${minTokens} - ${maxTokens} tokens per address\n`);

        // Helper function to generate random amount
        function getRandomAmount() {
            const randomTokens = Math.floor(Math.random() * (maxTokens - minTokens + 1)) + minTokens;
            return web3.utils.toWei(randomTokens.toString(), "ether");
        }

        // Store amounts for verification
        let totalMyToken = 0;
        let totalOtherToken = 0;
        const mintDetails = [];

        // ✅ Mint MyToken (token0) to REMAINING addresses (starting from index 20)
        // console.log("💰 Minting MyToken (token0) with RANDOM amounts (starting from address 21)...");
        for (let i = 0; i < remainingAddresses.length; i++) {
            const address = remainingAddresses[i];
            const randomAmount = getRandomAmount();
            const tokenAmount = parseFloat(web3.utils.fromWei(randomAmount, "ether"));
            
            try {
                await myToken.mintRewards(address, randomAmount, { from: owner });
                totalMyToken += tokenAmount;
                
                // Store for later verification
                mintDetails[i] = { address, myToken: tokenAmount };
                ...
                if ((i + 1) % 10 === 0 || i === remainingAddresses.length - 1) {
                    console.log(`  ✅ Minted to ${i + 1}/${remainingAddresses.length} addresses (avg: ${(totalMyToken / (i + 1)).toFixed(2)} tokens)`);
                }
            } catch (error) {
                console.error(`  ❌ Failed to mint to ${address}: ${error.message}`);
            }
        }

        // Verify balances for first 10 NEW addresses (indices 20-29)
        console.log("\n🔍 Verifying random amounts of the addresses:");
        for (let i = 0; i < remainingAddresses.length; i++) {
            const address = remainingAddresses[i];
            const balance0 = await myToken.balanceOf(address);
            const balance1 = await otherToken.balanceOf(address);
            
            console.log(`\n  Address ${startIndex + i + 1}: ${address.substring(0, 10)}...${address.substring(38)}`);
            console.log(`    MyToken: ${parseFloat(web3.utils.fromWei(balance0, "ether")).toFixed(2)} tokens`);
            console.log(`    OtherToken: ${parseFloat(web3.utils.fromWei(balance1, "ether")).toFixed(2)} tokens`);
        }

        // Summary statistics
        console.log("\n📊 SUMMARY STATISTICS:");
        console.log(`  Addresses skipped (already have token0): ${startIndex}`);
        console.log(`  Addresses minted in this run: ${remainingAddresses.length}`);
        console.log(`  MyToken:`);
        console.log(`    - Total minted: ${totalMyToken.toFixed(2)} tokens`);
        console.log(`    - Average per address: ${(totalMyToken / remainingAddresses.length).toFixed(2)} tokens`);
        console.log(`    - Range: ${minTokens} - ${maxTokens} tokens`);
        console.log(`  OtherToken:`);
        console.log(`    - Total minted: ${totalOtherToken.toFixed(2)} tokens`);
        console.log(`    - Average per address: ${(totalOtherToken / remainingAddresses.length).toFixed(2)} tokens`);

        console.log("\n✅ Token minting with RANDOM amounts completed successfully!\n");
